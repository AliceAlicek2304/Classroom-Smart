...
public class ClassroomService {
    ...
    public ClassroomResponse updateClassroom(Long id, ClassroomRequest request) {
        ...
        // Check permissions: ADMIN can update any, TEACHER can only update their own
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        Account currentUser = accountRepository.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("User not found"));

        if (currentUser.getRole() != Role.ADMIN && !classroom.getTeacher().getId().equals(currentUser.getId())) {
            throw new RuntimeException("You do not have permission to update this classroom");
        }
    ...
    public void deleteClassroom(Long id) {
        ...
        if (currentUser.getRole() != Role.ADMIN && !classroom.getTeacher().getId().equals(currentUser.getId())) {
            throw new RuntimeException("You do not have permission to delete this classroom");
        }
    ...
    public StudentInClassResponse addStudent(Long classroomId, AddStudentRequest request) {
        ...
        if (classStudentRepository.existsByClassroomIdAndStudentIdAndIsActive(classroomId, request.getStudentId(),
                true)) {
    ...
    public StudentInClassResponse enrollWithPassword(Long classroomId, String password) {
        ...
        // N\u1ebfu l\u1edbp c\u00f3 \u0111\u1eb7t m\u1eadt kh\u1ea9u th\u00ec ki\u1ec3m
        // tra, null = m\u1edf (ch\u1ea5p nh\u1eadn m\u1ecdi m\u1eadt kh\u1ea9u)
        ...
        Optional<ClassStudent> existing = classStudentRepository.findByClassroomIdAndStudentId(classroomId,
                student.getId());
        ClassStudent classStudent;
